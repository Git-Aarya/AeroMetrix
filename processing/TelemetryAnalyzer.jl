...
function process_flight_data(file_path::String)    
    ...
    # Wind Resistance is the difference between Airspeed and Groundspeed
    df.WindResistance = df.Airspeed_ms .- df.Groundspeed_ms
    avg_wind_res = sum(df.WindResistance) / nrow(df)
    
    # Calculate battery voltage drop over the flight
    ...
    # Simplified simulation metric: mV drop per minute converted to "mAh" drain factor roughly
    avg_drain = (voltage_drop / total_time_mins) * 1000 
    
    # Calculate Peak Wind Resistance
    peak_wind_res = maximum(df.WindResistance)

    # Output as JSON so C# can parse it easily
...
# Check if script is run directly from command line (e.g. by C# Process)
